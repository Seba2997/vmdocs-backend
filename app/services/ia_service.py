...
def generar_ficha(db: Session, documento_id: int, forzar: bool = False) -> Documento:
    ...
    prompt = (
        "Analiza el siguiente documento jurídico y extrae los datos solicitados con la mayor precisión posible.\n\n"
        "Devuelve un objeto JSON con exactamente estas claves:\n"
        "- tipo_documento: string con el tipo de documento (Ej: Contrato de Arrendamiento, Resolución Judicial, Escritura Pública), "
        "o null si no se puede determinar de forma segura.\n"
        "- partes_involucradas: lista de strings con los nombres completos de las personas naturales o jurídicas mencionadas. "
        "Incluye su rol si es evidente (Ej: 'Juan Pérez (Arrendador)'). Lista vacía [] si no hay.\n"
        "- fechas_importantes: lista de strings con fechas clave mencionadas en el documento, indicando su contexto "
        "(Ej: '2023-10-01 (Fecha de firma)'). Usa el formato YYYY-MM-DD cuando sea posible. Lista vacía [] si no hay fechas.\n"
        "- montos_detectados: lista de strings con montos o valores económicos detectados, indicando su moneda y propósito "
        "(Ej: '$1.000.000 CLP (Renta mensual)'). Lista vacía [] si no hay.\n"
        "- resumen_corto: string muy conciso, de máximo 2 oraciones, resumiendo el propósito principal y objeto del documento, "
        "o null si no hay suficiente información.\n\n"
        "Reglas estrictas:\n"
        "- Extrae SOLO información que aparezca explícitamente en el documento.\n"
        "- NUNCA inventes datos, nombres, fechas ni montos. Si un campo no existe o hay duda, usa null o [].\n"
        "- Las listas deben contener únicamente strings.\n"
        "- Normaliza los textos: usa mayúsculas iniciales para nombres propios y da formato claro a los montos.\n\n"
        "## Documento:\n"
        f"{texto}"
    )
